...
CellFunMap provides built-in reference topic models for users to take advantage of large landscape datasets such as the human cell landscape (HCL) and the mouse cell atlas (MCA).

```{r, eval = FALSE}
?HCL_ldaOut
?MCA_ldaOut
# SeuratObj <- transfer_LDA(query_SeuratObj = SeuratObj, ref_ldaOUt = HCL_ldaOut, by = "GO")
```

CellFunMap also provides methods for users to make their own reference topic models. 
...
unique(Seurat::Idents(reference_SeuratObj))
```
...
pheatmap::pheatmap(mm, cluster_rows = T, cluster_cols = T, angle_col = "0", main = "probability between topic and cluster",
                   ...
                   fontsize_row = 8, width = 9, height = 9)
```

## Visualize correlation or comparison

CellFunMap provides a few methods to visualize correlation or comparison between reference data and query data.
